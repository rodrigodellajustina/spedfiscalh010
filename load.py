...
class Sped:
    ...
    def condir(self):
        if self._dir != "":
            ...
            self._h010  = (self._dir + "/" + "h010.txt")
            self._h010r = (self._dir + "/" + "h010r.txt")
            self._proc1 = (self._dir + "/" + "proc1.txt")
            self._proc2 = (self._dir + "/" + "proc2.txt")
            self._proc3 = (self._dir + "/" + "proc3.txt")
            self._procrevision = (self._dir + "/" + "procrevision.txt")
            self._proc   = (self._dir + "/" + self._path+".proc")
        else:
...
sped = Sped(val_entrada=217780.67,  val_saida=865327.70, directory="", file="VAL_EFD_0002_Fevereiro_2022.sped")
...
if sped._dir_exist:
    ...
    df2["export"] = "|"+df2['a2'] + "|"+df2['product'] + "|"+df2['unidade'] + "|" + df2['qtd'] + "|" + df2['preco'] + "|" + df2['total'] + "|0|" + "|"+df2['descricao'].str.replace('"', "'")+"|" + df2['ncm'] + "|0|"
    ...
    with open(sped._dir, 'r') as fp:
        # read an store all lines into list
        lines = fp.readlines()

    with open(sped._h010r, 'r') as fp2:
        # read an store all lines into list
        lines2 = fp2.readlines()

    # Arquivo 01
    with open(sped._proc1, 'w') as fp:
        # iterate each line
    ...
    with open(sped._proc1) as fp:
        data1 = fp.read()

    with open(sped._proc2) as fp:
        data2 = fp.read()

    # Reading data from file2
    with open(sped._proc3) as fp:
        data3 = fp.read()

    data1 += data2
    data1 += data3

    with open (sped._procrevision, 'w') as fp:
        fp.write(data1)


    with open(sped._procrevision, 'r') as fp:
        # read an store all lines into list
        lines = fp.readlines()

    with open(sped._proc, 'w') as fp:
        for number, line in enumerate(lines):
            if "|" in line:
                fp.write(line)